/*
 * Cios Openapi
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: 1.0.1
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cios

import (
	"encoding/json"
)

// RouteContents struct for RouteContents
type RouteContents struct {
	OrderNo *int32 `json:"order_no,omitempty"`
	Type *string `json:"type,omitempty"`
	Id *string `json:"id,omitempty"`
}

// NewRouteContents instantiates a new RouteContents object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRouteContents() *RouteContents {
	this := RouteContents{}
	return &this
}

// NewRouteContentsWithDefaults instantiates a new RouteContents object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRouteContentsWithDefaults() *RouteContents {
	this := RouteContents{}
	return &this
}

// GetOrderNo returns the OrderNo field value if set, zero value otherwise.
func (o *RouteContents) GetOrderNo() int32 {
	if o == nil || o.OrderNo == nil {
		var ret int32
		return ret
	}
	return *o.OrderNo
}

// GetOrderNoOk returns a tuple with the OrderNo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RouteContents) GetOrderNoOk() (*int32, bool) {
	if o == nil || o.OrderNo == nil {
		return nil, false
	}
	return o.OrderNo, true
}

// HasOrderNo returns a boolean if a field has been set.
func (o *RouteContents) HasOrderNo() bool {
	if o != nil && o.OrderNo != nil {
		return true
	}

	return false
}

// SetOrderNo gets a reference to the given int32 and assigns it to the OrderNo field.
func (o *RouteContents) SetOrderNo(v int32) {
	o.OrderNo = &v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *RouteContents) GetType() string {
	if o == nil || o.Type == nil {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RouteContents) GetTypeOk() (*string, bool) {
	if o == nil || o.Type == nil {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *RouteContents) HasType() bool {
	if o != nil && o.Type != nil {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *RouteContents) SetType(v string) {
	o.Type = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *RouteContents) GetId() string {
	if o == nil || o.Id == nil {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RouteContents) GetIdOk() (*string, bool) {
	if o == nil || o.Id == nil {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *RouteContents) HasId() bool {
	if o != nil && o.Id != nil {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *RouteContents) SetId(v string) {
	o.Id = &v
}

func (o RouteContents) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.OrderNo != nil {
		toSerialize["order_no"] = o.OrderNo
	}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	if o.Id != nil {
		toSerialize["id"] = o.Id
	}
	return json.Marshal(toSerialize)
}

type NullableRouteContents struct {
	value *RouteContents
	isSet bool
}

func (v NullableRouteContents) Get() *RouteContents {
	return v.value
}

func (v *NullableRouteContents) Set(val *RouteContents) {
	v.value = val
	v.isSet = true
}

func (v NullableRouteContents) IsSet() bool {
	return v.isSet
}

func (v *NullableRouteContents) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRouteContents(val *RouteContents) *NullableRouteContents {
	return &NullableRouteContents{value: val, isSet: true}
}

func (v NullableRouteContents) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRouteContents) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


